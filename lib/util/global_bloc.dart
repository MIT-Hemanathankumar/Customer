...
import 'package:user/model/medicine_new.dart';
...
class GlobalBloc {
  ...
  BehaviorSubject<List<MedicineNew>> _medicineList$;
  BehaviorSubject<List<MedicineNew>> get medicineList$ => _medicineList$;

  GlobalBloc() {
    _medicineList$ = BehaviorSubject<List<MedicineNew>>.seeded([]);
    makeMedicineList();
  ...
  Future removeMedicine(MedicineNew tobeRemoved) async {
  ...
  Future updateMedicineList(MedicineNew newMedicine) async {
    var blocList = _medicineList$.value;
  ...
  Future makeMedicineList() async {
    ...
    List<MedicineNew> prefList = [];
    if (jsonList == null) {
      return;
    } else {
      for (String jsonMedicine in jsonList) {
        Map userMap = jsonDecode(jsonMedicine);
        MedicineNew tempMedicine = MedicineNew.fromJson(userMap);
